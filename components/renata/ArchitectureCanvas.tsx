...
export function ArchitectureCanvas({ diagramCode }: ArchitectureCanvasProps) {
  ...
  // Render the Mermaid diagram with Renata theme
  useEffect(() => {
    ...
    const renderDiagram = async () => {
      ...
      mermaid.initialize({
        ...
        theme: 'dark',
        themeVariables: {
          // Renata theme palette
          primaryColor: '#201e1a', // sys-surface
          primaryTextColor: '#e6e2de', // sys-text
          primaryBorderColor: '#ecc14f', // sys-primary
          lineColor: '#9a907c', // sys-muted
          secondaryColor: '#1a1815', // sys-elevated
          tertiaryColor: '#141210', // sys-surface
      ...
      try {
        const id = `renata-mermaid-${Date.now()}`;
      ...
      try {
        const id2 = `renata-mermaid-fallback-${Date.now()}`;
  ...
  return (
    <div className="relative w-full h-[600px] bg-sys-bg border border-sys-border rounded-lg overflow-hidden">
      ...
      >
        ...
        >
          ...
          {/* Loading state */}
          {!rendered && !error && (
            <div className="flex flex-col items-center gap-3 text-sys-muted">
              <Layers size={48} className="text-sys-primary/40 animate-pulse" />
              <p className="font-mono text-sm">Charting topology...</p>
            </div>
          )}
          ...
          {error && (
            <div className="max-w-2xl p-4">
              <p className="font-mono text-xs text-sys-warning mb-2">⚠ {error}</p>
              <pre className="font-mono text-xs text-sys-muted bg-sys-surface p-4 rounded border border-sys-border overflow-x-auto whitespace-pre-wrap max-h-80 overflow-y-auto">
      ...
      <div className="absolute bottom-4 right-4 flex flex-col gap-2">
        <button
          onClick={zoomIn}
          className="w-9 h-9 bg-sys-elevated border border-sys-border rounded flex items-center justify-center text-sys-text hover:border-sys-primary/40 hover:text-sys-primary transition-colors cursor-pointer"
          aria-label="Zoom in"
        >
                    <Plus size={18} />
        </button>
